...
class ReplyManager: NSObject {
        static let shared = ReplyManager()
        
        let ref = Database.database().reference(withPath: "replies")
        ...
        func notifySuccess(commentsContainer:CommentsContainerView){
            AlertManager.showAlert(title: "Success", message: "Reply posted", target: commentsContainer.parentViewController!)
        }
        ...
        func loadComments(for postID:String, VC:CommentsVC){
            ...
            ref.observe( .value, with: { snapshot in
                if snapshot.exists() == false {
                    (VC.view as! ReplyContainerView).collectionView.reloadData()
                    return
                }
...
extension ReplyManager:UICollectionViewDelegate, UICollectionViewDelegateFlowLayout, UICollectionViewDataSource{
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return comments.count
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "ReplyCell", for: indexPath) as! ReplyView
        cell.setup()
